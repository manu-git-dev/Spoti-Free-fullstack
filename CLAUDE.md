...
démarre les serveurs, joue les 169 tests, compile le build et vérifie `npm audit`.
...
`cd tests && npm install && npm test` — **169 tests** contre l'application réellement démarrée
...
- **Largeurs** (tranché le 2026-07-17, après trois essais — ne pas rejouer le débat) : l'**en-tête
  ...
  - **Mieux qu'un panneau, quand la page s'y prête : deux colonnes.** `Deposer.jsx` en est le
    modèle — formulaire à gauche, panneau « À savoir » à droite. C'est le seul agencement qui
    *remplit* vraiment, parce qu'il met du contenu utile dans l'espace au lieu de l'étirer. Ne
    s'applique pas à la prose, qui n'a rien à mettre à droite.
